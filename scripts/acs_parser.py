...
def extractor(url,driver,wait_time):
    ...
    abstract = soup.find('p', {'class': "articleBody_abstractText"}).text
...
data = pd.read_csv('acs_scrape.txt',header=None,names=['url']) # 'sd_test.txt' is the local file containing a list of the scraped urls
